...
val systemInformationInput = Source.fromFile("/home/bd-user/Downloads/system_information.json").mkString
  ...
  stmt.execute("""CREATE EXTERNAL TABLE s19909_bixi_feed_dhrumil.station_information ( station_id String, external_id STRING, name STRING, short_name STRING, lat FLOAT, lon FLOAT, rental_methods Array<STRING>, capacity INT, electric_bike_surcharge_waiver BOOLEAN, eightd_has_key_dispenser BOOLEAN, has_kiosk BOOLEAN )
                 |ROW FORMAT DELIMITED
                 |FIELDS TERMINATED BY ","
